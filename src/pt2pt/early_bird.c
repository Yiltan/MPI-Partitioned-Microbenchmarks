...
main(int argc, char **argv)
{
  ...
  double *total_communication_time_before_join;
  ...
  total_communication_time_before_join = (double*) calloc(num_iters, dsize);
  ...
  for (int message_size = min; message_size<=max; message_size *= 2) {
    ...
    if (myrank == source) {
      mean_fork_join_latency = mean(&total_communication_time[config.warmup_iterations],
                                    config.iterations);
    }
    ...
    for (int itr=0; itr<num_iters; itr++) {
      ...
      if (myrank == source) {
        ...
        total_communication_time_before_join[itr] = 0.0;
        for (int i=0; i<num_non_overlapped_intervals; i++) {
          total_communication_time[itr] += non_overlapped_intervals[i].end - non_overlapped_intervals[i].start;

          if (non_overlapped_intervals[i].start < mean_fork_join_latency) {
            if (non_overlapped_intervals[i].end < mean_fork_join_latency) {
              total_communication_time_before_join[itr] = total_communication_time_before_join[itr]
    ...
    if (myrank  == source)
    {
      ...
      double ave_communication_time = mean(&total_communication_time[config.warmup_iterations],
                                             config.iterations);
      double ave_communication_time_before_join
        = mean(&total_communication_time_before_join[config.warmup_iterations],
               config.iterations);

      double overlap = ave_communication_time_before_join / ave_communication_time;
      overlap = overlap * 100.0;

      printf("%-*d%*lf\n",
             WIDTH, message_size,
             (int) strlen("Overlap %%"), overlap);
    }
